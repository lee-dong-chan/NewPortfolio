...
const SkillsComp = ({ icons }: IProps) => {
  ...
  return (
    <div
      className={clsx(
        "mx-auto relative pt-[4rem] max-w-[1440px] cursor-pointer border-t border-indigo-950",
        "laptop:max-w-[1200px]"
      )}
    >
      ...
      <div className={"max-w-[1440px] px-[1rem]  py-[8rem] overflow-hidden"}>
        <motion.div
          key={slide ? "on" : "off"}
        ...
        >
          {icons.map((item: IICon, idx: number) => (
            ...
            >
              <div
                key={idx}
                className="flex flex-col items-center  p-[1rem] w-[100%] h-[100%] rounded-[0.5rem] bg-indigo-950"
              >
                <div className="w-[5rem] h-[5rem]">
                  <img className="w-[100%] h-[100%]" src={item.icon}></img>
                </div>
